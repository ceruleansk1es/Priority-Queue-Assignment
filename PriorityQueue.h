...
private:
    ...
    void upheap(size_type index) {
		...
		//check if max heap property isn't violated
        if(comp(c[parent_index], c[index])){
			//swap to move larger value upward
            std::swap(c[parent_index], c[index]);
			//keep bringing value up heap recursively
            upheap(parent_index);
        }
    }
    ...
    void downheap(size_type index) {
        // TODO
		//if the node is a leaf, t's already in the correct place
        if(is_leaf(index)){
            return;
        }
		...
		//initially assume that left child is larger
        size_t max_child_index = left_index;

        if(right_index < c.size()){
			//check if right child is larger than left child
            if(comp(c[left_index], c[right_index])){
                max_child_index = right_index;
            }
        }

		//check if current node is smaller than larger child
        if(comp(c[index], c[max_child_index])){
			//swap nodes to not violate max heap property
            std::swap(c[index], c[max_child_index]);
			//keep pushing value down heap recursively
